...
pub fn start(
    ...
    for binding in parse_bridge_channels(bridge_channels) {
        children.push(spawn_listener_publisher(
            pool.get().unwrap(),
            amqp_uri.to_string(),
            binding,
...
fn spawn_listener_publisher(
    ...
    thread::spawn(move || {
        ...
        println!("Send to {}...", binding.amqp_entity);
        ...
        headers.insert(
            "lang".to_string(),
            amqp::TableEntry::LongString("py".to_string()),
        );
        headers.insert(
            "task".to_string(),
            amqp::TableEntry::LongString("catch_change_events".to_string()),
        );

        while let Ok(Some(notification)) = it.next() {
            let (routing_key, message) = parse_notification(&notification.payload);
            let (exchange, _) = if amqp_entity_type == Type::Exchange {
                (binding.amqp_entity.as_str(), routing_key)
            } else {
                ("", binding.amqp_entity.as_str())
            };
            let task_id = Uuid::new_v4().to_hyphenated();
            let raw_message = format!("[[],{{\"message\":{}, \"route\":\"{}\"}},{{\"callbacks\":null,\"errbacks\":null,\"chain\":null,\"chord\":null}}]", message, routing_key);

            let mut current_headers = headers.clone();
            current_headers.insert(
                "id".to_string(),
                amqp::TableEntry::LongString(task_id.to_string()),
            );
...
*/
fn get_amq_entity_type(
    queue_channel: &mut Channel,
    exchange_channel: &mut Channel,
    amqp_entity: &str,
) -> Option<Type> {
    let opt_queue_type = queue_channel
        .queue_declare(
            amqp_entity.clone(),
            true,
            false,
        ...
        .map(|_| Type::Queue)
        .ok();
    let opt_exchange_type = exchange_channel
        .exchange_declare(
            amqp_entity,
            "",
            true,
...
fn parse_bridge_channels(bridge_channels: &str) -> Vec<Binding> {
    ...
    let strs: Vec<Vec<&str>> = bridge_channels
        .split(",")
        .map(|s| s.split(":").collect())
        .collect();
    for i in 0..strs.len() {
        bindings.push(Binding {
            pg_channel: strs[i][0].trim().to_string(),
            amqp_entity: strs[i].get(1).unwrap_or(&"").trim().to_string(),
        });
    }
    let mut cleaned_bindings: Vec<Binding> = bindings
        .into_iter()
        .filter(|x| !x.pg_channel.is_empty() && !x.amqp_entity.is_empty())
        .collect();
    if cleaned_bindings.len() == 0 {
        panic!(
            "No bindings(e.g. pgchannel1:queue1) specified in \"{}\"",
            bridge_channels
        )
    }
...
pub fn wait_for_amqp_session(amqp_uri: &str, pg_channel: &str) -> Session {
    println!(
        "Attempting to obtain connection on AMQP server for {} channel..",
        pg_channel
    );
    ...
    while let Err(e) = s {
        ...
        println!(
            "Retrying the AMQP connection for {} channel in {:?} seconds..",
            pg_channel,
            time.as_secs()
        );
        thread::sleep(time);
    ...
    println!(
        "Connection to AMQP server for {} channel successful",
        pg_channel
    );
    s.unwrap()
}
...
mod tests {
    ...
    fn parse_notification_works() {
        ...
        assert!(("my_key", "A message") == parse_notification("  my_key  |  A message  "));
        assert!(
            ("my_key", "A message|Rest of message")
                == parse_notification("my_key|A message|Rest of message")
        );
        assert!(("", "my_key##A message") == parse_notification("my_key##A message"));
    ...
    fn parse_bridge_channels_works() {
        assert!(
            vec![Binding {
                pg_channel: "pgchannel1".to_string(),
                amqp_entity: "exchange1".to_string()
        ...
        assert!(
            vec![
                ...
                Binding {
                    pg_channel: "pgchannel2".to_string(),
                    amqp_entity: "exchange2".to_string()
                }
        ...
        assert!(
            vec![
                ...
                Binding {
                    pg_channel: "pgchannel3".to_string(),
                    amqp_entity: "exchange3".to_string()
                }
            ] == parse_bridge_channels(
                " pgchannel1 : exchange1 , pgchannel2 : exchange2 , pgchannel3 : exchange3, "
            )
        );
    }
    ...
    fn parse_bridge_channels_panics_if_no_pg_channel_and_exchange_specified() {
        ...
        assert!(catch_unwind(|| parse_bridge_channels("pgchannel1:")).is_err());
        assert!(
            catch_unwind(|| parse_bridge_channels("pgchannel1, pgchannel1:, :exchange3,,"))
                .is_err()
        );
    }
    ...
    fn parse_bridge_channels_panics_if_duplicate_pg_channel() {
        assert!(catch_unwind(|| parse_bridge_channels(
